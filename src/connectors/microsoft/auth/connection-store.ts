...
export async function saveMicrosoftConnection(profile: { id: string; mail?: string; userPrincipalName: string }, token: { access_token: string; refresh_token?: string; expires_in: number; scope: string }) {
  ...
  const { data: connection, error } = await supabase.from("connections").upsert({ organization_id: org.id, owner_party_id: ownerPartyId, provider: "microsoft", auth_type: "oauth", owner_type: "service", status: "connected", capabilities: ["files.search","files.read","email.search","email.read","email.reply","email.replyAll","email.forward","email.send"], provider_tenant_id: env.MICROSOFT_TENANT_ID, provider_account_id: profile.id, account_address: accountAddress, scopes: token.scope.split(" "), expires_at: expiresAt, updated_at: new Date().toISOString() }, { onConflict: "organization_id,provider,provider_account_id" }).select("id,organization_id,owner_party_id").single();
