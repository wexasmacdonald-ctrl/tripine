...
export async function GET() {
  ...
  const [{ data: agents }, { data: connections }, { data: subscriptions }, { count: failedDeliveries }] = await Promise.all([
    admin.from("agents").select("id").eq("organization_id", membership.organization_id).eq("active", true),
    admin.from("connections").select("id,status,account_address").eq("organization_id", membership.organization_id).eq("provider", "microsoft"),
  ...
  const checks = { database: true, agentSeeded: Boolean(agents?.length), openai: Boolean(env.OPENAI_API_KEY), microsoftEnvironment: Boolean(env.MICROSOFT_CLIENT_ID && env.MICROSOFT_CLIENT_SECRET && env.MICROSOFT_GRAPH_CLIENT_STATE), encryption: Boolean(env.CREDENTIAL_ENCRYPTION_KEY), alexConnected: connected, webhookSubscribed: subscribed, failedDeliveries: failedDeliveries ?? 0 };
