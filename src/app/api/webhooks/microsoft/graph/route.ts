...
export async function POST(request: Request) {
  ...
  if (accepted.length && env.NEXT_PUBLIC_SUPABASE_URL && env.SUPABASE_SERVICE_ROLE_KEY) {
    const supabase = createAdminClient();
    const rows = accepted.map((item) => ({ provider: "microsoft", subscription_external_id: item.subscriptionId, provider_resource_id: item.resourceData?.id, payload: item, status: "pending" }));
    const { error } = await supabase.from("inbound_deliveries").upsert(rows, { onConflict: "provider,subscription_external_id,provider_resource_id" });
